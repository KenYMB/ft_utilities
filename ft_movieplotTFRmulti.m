function [cfg] = ft_movieplotTFRmulti(cfg, data)
...
% 
%   cfg.zlim         = plotting limits for color dimension, 'maxmin',
%                          'maxabs', 'zeromax', 'minzero', 'zeromaxabs' or [zmin zmax] (default = 'maxmin')
%   cfg.colormap     = ex)'jet' (default = 'default')
%   cfg.subplot      = [rows columns] (default = 'auto')
%   cfg.title        = {'title list'} (default = [])
%   cfg.titlesize    = number (default = 9)
...
% 20160812 Yuasa: for multiple movie, add 'aeromaxabs' for zlim,
...
for l = 1:numel(data)
    data{l} = ft_checkdata(data{l}, 'datatype', {'timelock', 'freq'});
end
...
xparam = 'time';
if isfield(data{1}, 'freq')
  yparam = 'freq';
end

% read or create the layout that will be used for plotting:
for l = 1:numel(data)
    if isempty(strfind(data{l}.label{1},'+'))
        cfg.layout = cfg.layouts{1};
    else
        cfg.layout = cfg.layouts{2};
    end
    layout{l} = ft_prepare_layout(cfg, data{l});
end
...
if ~strcmp(cfg.baseline, 'no')
  ...
  for l = 1:numel(data)
      data{l} = ft_freqbaseline(tmpcfg, data{l});
      [cfg, data{l}] = rollback_provenance(cfg, data{l});
  end
end

% prepare subplot parameter
if ~isfield(cfg,'subplot') || numel(cfg.subplot) < 2 || prod(cfg.subplot(1:2)) < numel(data)
    cfg.subplot = [];
    cfg.subplot = [ceil(sqrt(numel(data))), ceil(numel(data)/ceil(sqrt(numel(data))))];
end

% prepare title
if ~iscell(cfg.title),      cfg.title   = {cfg.title};  end
if ~iscellstr(cfg.title),   cfg.title   = [];
else                        cfg.title   = reshape(cfg.title',[],1);
    if length(cfg.title) < numel(data)
        cfg.title((length(cfg.title)+1):numel(data))   = {''};
    end
end
...
xvalues   = data{l}.(xparam);
parameter = data{l}.(cfg.parameter);
if exist('yparam', 'var') && ~isnan(yparam)
  yvalues = data{l}.(yparam);
end

% check consistency of xparam and yparam
% NOTE: i set two different defaults for the 'chan_time' and the 'chan_freq_time' case
if isfield(data{l},'dimord')
  if strcmp(data{l}.dimord,'chan_freq_time')
  ...
  elseif strcmp(data{l}.dimord,'chan_time')
...
[seldat, sellay] = match_str(data{l}.label, layout{l}.label);
if isempty(seldat)
...
chanx = layout{l}.pos(sellay,1);
chany = layout{l}.pos(sellay,2);

% get the z-range
if ischar(cfg.zlim) && strcmp(cfg.zlim, 'maxmin')
  cfg.clim    = [];
  cfg.clim(1) = min(parameter(:));
  cfg.clim(2) = max(parameter(:));
elseif ischar(cfg.zlim) && strcmp(cfg.zlim,'maxabs')
  cfg.clim     = [];
  cfg.clim(1)  = -max(abs(parameter(:)));
  cfg.clim(2)  =  max(abs(parameter(:)));
...
elseif ischar(cfg.zlim) && strcmp(cfg.zlim,'minzero')
  ...
  cfg.clim(1)  = min(parameter(:));
  cfg.clim(2)  = 0;
elseif ischar(cfg.zlim) && strcmp(cfg.zlim,'zeromaxabs')
  cfg.clim     = [];
  cfg.clim(1)  = -max(abs(parameter(:)));
  cfg.clim(2)  =  max(abs(parameter(:)));
  if min(parameter(:)) > 0
      cfg.clim(1)  = 0;
  elseif max(parameter(:)) < 0
      cfg.clim(0)  = 0;
  end
end
...
if dointeractive
  ...
  opt{numel(data)+1}.speed    = 1;
  opt{numel(data)+1}.sx       = sx; % slider freq
  opt{numel(data)+1}.sy       = sy; % slider time
  opt{numel(data)+1}.p        = p;
  opt{numel(data)+1}.t        = t;
  if ~hasyparam
    opt{numel(data)+1}.timdim = 2;
  else
    opt{numel(data)+1}.timdim = 3;
  end
  end
  ...
  opt{l}.chanx    = chanx;
  opt{l}.chany    = chany;
  opt{l}.xvalues  = xvalues; % freq
  opt{l}.yvalues  = yvalues; % time
  opt{l}.xparam   = xparam;
  opt{l}.yparam   = yparam;
  opt{l}.dat      = parameter;
  opt{l}.zlim     = cfg.clim;
  opt{l}.cfg      = cfg;
  opt{l}.colorbar = istrue(cfg.colorbar);
  opt{l}.colormap = cfg.colormap;
  subplot(cfg.subplot(1),cfg.subplot(2), l)
  [dum, hs] = ft_plot_topo(chanx, chany, zeros(numel(chanx),1), 'mask', layout{l}.mask, 'outline', layout{l}.outline, 'interpmethod', 'v4', 'interplim', 'mask');
  opt{l}.ha  = gca;
  caxis(cfg.clim);
  ...
  if ~isempty(cfg.title)
      title(cfg.title{l},'FontSize',cfg.titlesize);
  end

  % add sum stuff at a higher level for quicker access in the callback
  % routine
  opt{l}.xdata   = get(hs, 'xdata');
  opt{l}.ydata   = get(hs, 'ydata');
  opt{l}.nanmask = get(hs, 'cdata');
  ...
  set(p, 'callback', @cb_playbutton);
    ...
    if l==1
        warning('non interactive mode does not work');
    end
...
function cb_slider(h, eventdata)
  ...
  xdim = opt{numel(opt)}.timdim;
  valx = get(opt{numel(opt)}.sx, 'value');
  valx = round(valx*(size(opt{l}.dat,xdim)-1))+1;
  valx = min(valx, size(opt{l}.dat,xdim));
  valx = max(valx, 1);
  if valx>size(opt{l}.dat,opt{numel(opt)}.timdim)
    valx = size(opt{l}.dat,opt{numel(opt)}.timdim)-1;
  end

for l = 1:numel(opt)-1
if length(size(opt{l}.dat))>2
  ydim = 2;
  valy = get(opt{numel(opt)}.sy, 'value');
  valy = round(valy*(size(opt{l}.dat,ydim)-1))+1;
  valy = min(valy, size(opt{l}.dat,ydim));
  valy = max(valy, 1);

  set(opt{l}.hx, 'string', sprintf('%s = %f\n', opt{l}.xparam, opt{l}.xvalues(valx)));
  set(opt{l}.hy, 'string', sprintf('%s = %f\n', opt{l}.yparam, opt{l}.yvalues(valy)));

  % update data, interpolate and render
  datamatrix = griddata(opt{l}.chanx, opt{l}.chany, opt{l}.dat(:,valy,valx), opt{l}.xdata, opt{l}.ydata, 'v4');
else
  set(opt{l}.hx, 'string', sprintf('%s = %f\n', opt{l}.xparam, opt{l}.xvalues(valx)));
  % update data, interpolate and render
  datamatrix = griddata(opt{l}.chanx, opt{l}.chany, opt{l}.dat(:,valx), opt{l}.xdata, opt{l}.ydata, 'v4');
end
set(opt{l}.hs, 'cdata',  datamatrix + opt{l}.nanmask);
end
...
switch get(h, 'string')
  case 'play'
    set(h, 'string', 'stop');
    start(opt{numel(opt)}.t);
  case 'stop'
    set(h, 'string', 'play');
    stop(opt{numel(opt)}.t);
end
...
l = 1;
delta = opt{numel(opt)}.speed/size(opt{l}.dat,opt{numel(opt)}.timdim);
val = get(opt{numel(opt)}.sx, 'value');
val = val + delta;
...
set(opt{numel(opt)}.sx, 'value', val);
cb_slider(h);
...
switch get(h, 'string')
  case '+'
    opt{numel(opt)}.speed = opt{numel(opt)}.speed*sqrt(2);
  case '-'
    opt{numel(opt)}.speed = opt{numel(opt)}.speed/sqrt(2);
