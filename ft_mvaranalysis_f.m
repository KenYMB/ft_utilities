...
elseif ~isempty(cfg.toi) && ~isempty(cfg.t_ftimwin)
  ...
  for k = 1:numel(cfg.toi)
		latency(k,:) = cfg.toi(k) + cfg.t_ftimwin.*[-0.5 0.5];
	end
else
...
tfwin    = round(data.fsample.*cfg.t_ftimwin) + 1;
...
for j = 1:ntoi
	...
	if ntoi>1 && strcmp(cfg.toolbox, 'bsmart')
  	...
  	cfg.toi(j) = round(mean(tmpdata.time{ix}([1 end])) * data.fsample) / data.fsample; %FIXED
	end
