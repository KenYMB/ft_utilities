function [res] = ft_mnefif_compute_raw_inverse_parallel(data,fname_inv,nave,lambda2,dSPM,sLORETA,labelfile)
%
% 
% parpool;                            % matlabpool open
% [res] = FT_MNEFIF_COMPUTE_RAW_INVERSE_PARALLEL(data,fname_inv,nave,lambda2,dSPM,sLORETA,labelfile)
...
% using: fieldtrip(, ft_hastoolbox, mne, match_str), parallel computing toolbox

% 20161212 Yuasa: create based on mne_ex_compute_inverse.m
%                 adapt for parallel computing
...
FIFF_ORI = FIFF.FIFFV_MNE_FREE_ORI;
...
pardata = data.trial;
parfor itrl=1:length(data.trial)
    pardata{itrl} = double(pardata{itrl}(selchan,:));
end
...
if isempty(selvert)
    ...
    for itrl=1:length(data.trial)
        sols{itrl} = zeros(inv.nsource,size(data.trial{1},2));
    end
    inv_diag_reginv      = diag(sparse(inv.reginv));
    inv_diag_sourcecov   = diag(sparse(sqrt(inv.source_cov.data(seltris,:))));
    inv_eigen_fields     = inv.eigen_fields.data;
    inv_iseigenleads     = inv.eigen_leads_weighted;
    inv_eigen_leads      = inv.eigen_leads.data(seltris,:);
    inv_whitener         = inv.whitener;
    inv_proj             = inv.proj;
    inv_source_ori       = inv.source_ori;
    inv_noisenorm        = inv.noisenorm;       % might be empty
    if dSPM || sLORETA
        inv_noisenorm    = inv_noisenorm(selvert,selvert);
    end
    ...
    parfor itrl=1:length(data.trial)
        ...
        %   eigenleads
        %   
        trans = inv_diag_reginv*inv_eigen_fields*inv_whitener*inv_proj*pardata{itrl};
        %
        ...
        if inv_iseigenleads
           ...
           sol   = inv_eigen_leads*trans;
        else
           %
           %     R^0.5 has to factored in
           %
           fprintf(1,'(eigenleads need to be weighted)...');
           sol   = inv_diag_sourcecov*inv_eigen_leads*trans;
        end

        if inv_source_ori == FIFF_ORI
