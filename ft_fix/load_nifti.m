...
% 20160816 Yuasa: applied for win
% 20170206 Yuasa: remove uncompressed file
...
if(strcmpi(ext,'.gz'))
  if ispc
    fprintf('Uncompressing %s into %s\n',niftifile,tempdir);
    new_niftifile = gunzip(niftifile,tempdir);
    new_niftifile = new_niftifile{1};
    [~,gzipped]   = fileparts(new_niftifile);
  else
