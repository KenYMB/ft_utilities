...
%   cfg.axis        = 'yes' or 'no', if show axis label (default = 'yes')
%
...
% 20170223 Yuasa: fix bugs
% 20170224 Yuasa: add cfg.legend, cfg.axis
% 20180725 Yuasa: additional bug fix (graphcolor)
...
elseif ischar(cfg.zlim) && strcmp(cfg.zlim,'maxabs')
  zmax = -inf;
  for k = 1:Ndata
    zmax = max(zmax,max(abs(varargin{k}.(cfg.parameter)(:))));
  end
  zmin = -zmax;
else
  zmin = cfg.zlim(1);
...
if Ndata>1
  ...
  for k = 2:Ndata
    ...
    elseif isnumeric(cfg.graphcolor),tmpcfg.graphcolor = tmpcfg.graphcolor(2:end,:);
    end
...
for k = 1:nchan
  for m = 1:nchan
    if k~=m
      ...
      if ischar(cfg.graphcolor),        graphcolor = cfg.graphcolor(1);
      elseif isnumeric(cfg.graphcolor), graphcolor = cfg.graphcolor(1, :);
      end
      ft_plot_vector(tmp, 'width', 1, 'height', 1, 'hpos', ix.*1.2, 'vpos', iy.*1.2, 'vlim', cfg.zlim, 'box', 'yes', 'color', graphcolor);
      if strcmp(cfg.axis, 'yes')
      if k==1,
