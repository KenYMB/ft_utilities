function [component cfg] = ft_databrowserICA(cfg, data)
...
% 
%   cfg.topocolormap            = any sized colormap or colormap name for topoplot (default = 'default')
%   cfg.combineplanar           = 'yes' or 'no' whther ft_combineplanar is applied to data.topo (default = 'no')
%   cfg.combine.***             = you can use configuration options of ft_combineplanar (default = [])
%
...
% using: fieldtrip, ft_private(ft_platform_supports, join_str)

% 20160805 Yuasa: customize for ica: add components button
% 20161128 Yuasa: auto save for 'q'
% 20161221 Yuasa: minor fix
% 20170110 Yuasa: minor fix
% 20170127 Yuasa: add automatically combine planar for topo
% 20171013 Yuasa: add detailed error massage for combineplanar
...
cfg.topocolormap    = ft_getopt(cfg, 'topocolormap', 'default');
cfg.combineplanar   = ft_getopt(cfg, 'combineplanar', 'no');
cfg.combine         = ft_getopt(cfg, 'combine', []);
...
% combine planar
if strcmpi(cfg.combineplanar,'yes')
    fprintf(1,'Applying ft_combineplanar\n');
    try
        if hascomp
            topodata        = [];
            topodata.trial  = {data.topo};
            topodata.label  = data.topolabel;
            try topodata.grad   = data.grad; 
            catch, try topodata.grad   = cfg.combine.grad;
                   catch, try topodata.grad   = cfg.grad;   end
                   end
            end
            topodata.time   = {1:size(data.topo,2)};
            topodata        = ft_combineplanar(cfg.combine,topodata);
            
            data.topo      = topodata.trial{1};
            data.topolabel = topodata.label;
        else
            data    = ft_combineplanar(cfg.combine,data);
        end 
    catch
        fprintf(2,'Error in ft_combineplanar.\nMaybe the input data lacks ''grad'' field.\n');
        warning(sprintf('Combineplanar is not applied.\nPlease combine planar sensors manually.'));
    end
    fprintf(1,'Done.\n');    
end
...
uicontrol('tag', 'components', 'parent', h, 'units', 'normalized', 'style', 'pushbutton', 'string', 'components', 'position', [0.91, 0.55 - ((iArt)*0.09), 0.08, 0.04], 'callback', @component_cb)
...
if nargout
  ...
  while ishandle(h)
    ...
    opt = getappdata(h, 'opt');
      ...
      try
          component = sort(str2num(getappdata(h, 'component')));
      catch
          component = [];
      end
...
% SUBFUNCTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function component_cb(h,eventdata)
parent = get(h, 'parent');

ppbh = getappdata(parent,'iscomponentwindow');  % get component window handle
if ~isempty(ppbh) && ishandle(ppbh)        % avoid second window
    figure(get(ppbh,'Parent'));
else
    ...
    axis off
    % add save button
    ppbh = uicontrol('tag', 'component_save', 'parent', pph, 'units', 'normalized', 'style', 'pushbutton', 'string', 'save and finish', 'position', [0.81, 0.6 , 0.18, 0.10], 'callback', @outputcomponent_cb);

    % add edit box
    ppeh = uicontrol('style', 'edit');
    set(pph, 'toolBar', 'none')
    set(pph, 'menuBar', 'none')
    set(pph, 'Name', 'artifact components editor')
    set(pph, 'NumberTitle', 'off')
    set(ppeh, 'Units', 'normalized');
    set(ppeh, 'Position', [0 0 .8 1]);
    set(ppeh, 'backgroundColor', [1 1 1]);
    ...
    setappdata(pph, 'superparent', parent); % superparent is the main ft_databrowser window
    setappdata(pph, 'ppeh', ppeh);
    setappdata(parent, 'iscomponentwindow', ppbh); % component window handle

end
end % function component_cb

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBFUNCTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function outputcomponent_cb(h,eventdata)
parent = get(h, 'parent');
superparent = getappdata(parent, 'superparent');
ppeh = getappdata(parent, 'ppeh');
code = get(ppeh, 'string');

  setappdata(superparent, 'component', code)
  
close(parent)

cleanup_cb(superparent);
end % function outputcomponent_cb
...
switch key
  ...
  case 'q'
    setappdata(h, 'opt', opt);
    setappdata(h, 'cfg', cfg);
    hcomp = getappdata(h,'iscomponentwindow');  % get component window handle
    if ~isempty(hcomp) && ishandle(hcomp)
        outputcomponent_cb(hcomp);
    end
    cleanup_cb(h);
  case 't'
...
try     colormap(cfg.topocolormap);     end
